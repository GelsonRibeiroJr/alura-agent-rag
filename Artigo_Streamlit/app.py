...
st.set_page_config(page_title="Assistente Técnico NASA", page_icon="🚀", layout="centered")

st.title("🚀 Assistente Técnico da NASA")
st.caption("Agente RAG especialista em exploração lunar e Programa Artemis com memória e citações de fontes.")

# --- CARREGAMENTO DO BANCO VETORIAL & LLM ---
@st.cache_resource
def carregar_recursos():
    embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")
    vectorstore = FAISS.load_local("Artigo_Streamlit/faiss_index", embeddings, allow_dangerous_deserialization=True)
...
try:
    ...
    st.error("Erro ao carregar o banco vetorial ou a API Key da Groq.")
    st.stop()
...
conn = sqlite3.connect("Artigo_Streamlit/memoria_agente.db", check_same_thread=False)
